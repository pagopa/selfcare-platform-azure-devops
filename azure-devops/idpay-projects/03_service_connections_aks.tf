resource "azuredevops_serviceendpoint_kubernetes" "aks_dev" {
  ...
  apiserver_url         = module.idpay_dev_secrets.values["cstar-d-weu-dev01-aks-apiserver-url"].value
  authorization_type    = "ServiceAccount"
  service_account {
    # base64 values
    token   = module.idpay_dev_secrets.values["cstar-d-weu-dev01-aks-azure-devops-sa-token"].value
    ca_cert = module.idpay_dev_secrets.values["cstar-d-weu-dev01-aks-azure-devops-sa-cacrt"].value
  }
}
