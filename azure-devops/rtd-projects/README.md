...
| <a name="input_rtd-ms-sender-auth"></a> [rtd-ms-sender-auth](#input\_rtd-ms-sender-auth) | n/a | `map` | <pre>{<br>  "pipeline": {<br>    "enable_code_review": true,<br>    "enable_deploy": true,<br>    "path": "rtd\\rtd-ms-sender-auth"<br>  },<br>  "repository": {<br>    "branch_name": "refs/heads/main",<br>    "name": "rtd-ms-sender-auth",<br>    "organization": "pagopa",<br>    "pipelines_path": ".devops",<br>    "yml_prefix_name": "cstar-rtd"<br>  }<br>}</pre> | no |
