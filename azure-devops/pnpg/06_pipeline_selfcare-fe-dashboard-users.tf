...
locals {
  ...
  selfcare-dashboard-users-microfrontend-variables_code_review = {
    ...
    sonarcloud_org          = var.selfcare-dashboard-users-microfrontend.repository.organization
    sonarcloud_project_key  = "${var.selfcare-dashboard-users-microfrontend.repository.organization}_${var.selfcare-dashboard-users-microfrontend.repository.name}"
    sonarcloud_project_name = var.selfcare-dashboard-users-microfrontend.repository.name
  }
  # code_review secrets
  ...
  selfcare-dashboard-users-microfrontend-variables_deploy = {
    ...
    K8S_IMAGE_REPOSITORY_NAME        = replace(var.selfcare-dashboard-users-microfrontend.repository.name, "-", "")
    DEPLOY_NAMESPACE                 = local.domain
    DEPLOYMENT_NAME                  = "dashboard-users-microfrontend"
    SETTINGS_XML_RW_SECURE_FILE_NAME = "settings-rw.xml"
    SETTINGS_XML_RO_SECURE_FILE_NAME = "settings-ro.xml"
    HELM_RELEASE_NAME                = var.selfcare-dashboard-users-microfrontend.repository.name
