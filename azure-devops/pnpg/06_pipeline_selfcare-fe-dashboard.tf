...
locals {
  ...
  selfcare-dashboard-frontend-variables_code_review = {
    ...
    sonarcloud_org          = var.selfcare-dashboard-frontend.repository.organization
    sonarcloud_project_key  = "${var.selfcare-dashboard-frontend.repository.organization}_${var.selfcare-dashboard-frontend.repository.name}"
    sonarcloud_project_name = var.selfcare-dashboard-frontend.repository.name
  }
  # code_review secrets
  ...
  selfcare-dashboard-frontend-variables_deploy = {
    ...
    K8S_IMAGE_REPOSITORY_NAME        = replace(var.selfcare-dashboard-frontend.repository.name, "-", "")
    DEPLOY_NAMESPACE                 = local.domain
    DEPLOYMENT_NAME                  = "dashboard-frontend"
    SETTINGS_XML_RW_SECURE_FILE_NAME = "settings-rw.xml"
    SETTINGS_XML_RO_SECURE_FILE_NAME = "settings-ro.xml"
    HELM_RELEASE_NAME                = var.selfcare-dashboard-frontend.repository.name
