variable "selfcare-fe-dashboard" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "selfcare-fe-dashboard"
    ...
    pipeline = {
      ...
      enable_deploy      = true
      path               = "pnpg\\selfcare-fe-dashboard"
    }
  }
}
...
locals {
  # global vars
  selfcare-fe-dashboard-variables = {
  ...
  # global secrets
  selfcare-fe-dashboard-variables_secret = {

  }
  # code_review vars
  selfcare-fe-dashboard-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.selfcare-fe-dashboard.repository.organization
    sonarcloud_project_key  = "${var.selfcare-fe-dashboard.repository.organization}_${var.selfcare-fe-dashboard.repository.name}"
    sonarcloud_project_name = var.selfcare-fe-dashboard.repository.name
  }
  # code_review secrets
  selfcare-fe-dashboard-variables_secret_code_review = {

  }
  # deploy vars
  selfcare-fe-dashboard-variables_deploy = {

    K8S_IMAGE_REPOSITORY_NAME        = replace(var.selfcare-fe-dashboard.repository.name, "-", "")
    DEPLOY_NAMESPACE                 = local.domain
    DEPLOYMENT_NAME                  = "fe-dashboard"
    SETTINGS_XML_RW_SECURE_FILE_NAME = "settings-rw.xml"
    SETTINGS_XML_RO_SECURE_FILE_NAME = "settings-ro.xml"
    HELM_RELEASE_NAME                = var.selfcare-fe-dashboard.repository.name
  ...
  # deploy secrets
  selfcare-fe-dashboard-variables_secret_deploy = {

  }
}

module "selfcare-fe-dashboard_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v2.6.5"
  count  = var.selfcare-fe-dashboard.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = data.azuredevops_project.project.id
  repository                   = var.selfcare-fe-dashboard.repository
  github_service_connection_id = data.azuredevops_serviceendpoint_github.github_ro.service_endpoint_id
  path                         = var.selfcare-fe-dashboard.pipeline.path

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.selfcare-fe-dashboard-variables,
    local.selfcare-fe-dashboard-variables_code_review,
  )

  variables_secret = merge(
    local.selfcare-fe-dashboard-variables_secret,
    local.selfcare-fe-dashboard-variables_secret_code_review,
  )
...
module "selfcare-fe-dashboard_deploy" {
  ...
  variables = merge(
    local.selfcare-fe-dashboard-variables,
    local.selfcare-fe-dashboard-variables_deploy,
  )

  variables_secret = merge(
    local.selfcare-fe-dashboard-variables_secret,
    local.selfcare-fe-dashboard-variables_secret_deploy,
  )
