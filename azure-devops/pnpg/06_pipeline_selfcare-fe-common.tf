variable "selfcare-fe-common" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "selfcare-fe-common"
...
locals {
  ...
  # global secrets
  selfcare-fe-common-variables_secret = {

  }
  # code_review vars
  selfcare-fe-common-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.selfcare-fe-common.repository.organization
    sonarcloud_project_key  = "${var.selfcare-fe-common.repository.organization}_${var.selfcare-fe-common.repository.name}"
    sonarcloud_project_name = var.selfcare-fe-common.repository.name
  }
  # code_review secrets
  selfcare-fe-common-variables_secret_code_review = {

  }
  # deploy vars
  selfcare-fe-common-variables_deploy = {

    K8S_IMAGE_REPOSITORY_NAME        = replace(var.selfcare-fe-common.repository.name, "-", "")
    DEPLOY_NAMESPACE                 = local.domain
    DEPLOYMENT_NAME                  = "fe-common"
    SETTINGS_XML_RW_SECURE_FILE_NAME = "settings-rw.xml"
    SETTINGS_XML_RO_SECURE_FILE_NAME = "settings-ro.xml"
    HELM_RELEASE_NAME                = var.selfcare-fe-common.repository.name
  ...
  # deploy secrets
  selfcare-fe-common-variables_secret_deploy = {

  }
}

module "selfcare-fe-common_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v2.6.5"
  count  = var.selfcare-fe-common.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = data.azuredevops_project.project.id
  repository                   = var.selfcare-fe-common.repository
  github_service_connection_id = data.azuredevops_serviceendpoint_github.github_ro.service_endpoint_id
  path                         = var.selfcare-fe-common.pipeline.path

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.selfcare-fe-common-variables,
    local.selfcare-fe-common-variables_code_review,
  )

  variables_secret = merge(
    local.selfcare-fe-common-variables_secret,
    local.selfcare-fe-common-variables_secret_code_review,
  )
...
module "selfcare-fe-common_deploy" {
  ...
  variables = merge(
    local.selfcare-fe-common-variables,
    local.selfcare-fe-common-variables_deploy,
  )

  variables_secret = merge(
    local.selfcare-fe-common-variables_secret,
    local.selfcare-fe-common-variables_secret_deploy,
  )
