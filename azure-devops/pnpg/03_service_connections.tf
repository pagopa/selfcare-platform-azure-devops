...
resource "azuredevops_serviceendpoint_dockerregistry" "github_docker_registry_ro" {
  ...
  docker_username       = module.secrets_core_prod.values["SELC-DOCKER-REGISTRY-PAGOPA-USER"].value
  docker_password       = module.secrets_core_prod.values["SELC-DOCKER-REGISTRY-PAGOPA-TOKEN-RO"].value
...
resource "azuredevops_serviceendpoint_npm" "pagopa-npm-bot" {
  ...
  access_token          = module.secrets_core_prod.values["pagopa-npm-bot-TOKEN"].value
}
