...
locals {
  ...
  selfcare-pnpg-onboarding-frontend-variables_code_review = {
    ...
    sonarcloud_org          = var.selfcare-pnpg-onboarding-frontend.repository.organization
    sonarcloud_project_key  = "${var.selfcare-pnpg-onboarding-frontend.repository.organization}_${var.selfcare-pnpg-onboarding-frontend.repository.name}"
    sonarcloud_project_name = var.selfcare-pnpg-onboarding-frontend.repository.name
  }
  # code_review secrets
  ...
  selfcare-pnpg-onboarding-frontend-variables_deploy = {
    ...
    K8S_IMAGE_REPOSITORY_NAME        = replace(var.selfcare-pnpg-onboarding-frontend.repository.name, "-", "")
    DEPLOY_NAMESPACE                 = local.domain
    DEPLOYMENT_NAME                  = "pnpg-onboarding-frontend"
    SETTINGS_XML_RW_SECURE_FILE_NAME = "settings-rw.xml"
    SETTINGS_XML_RO_SECURE_FILE_NAME = "settings-ro.xml"
    HELM_RELEASE_NAME                = var.selfcare-pnpg-onboarding-frontend.repository.name
